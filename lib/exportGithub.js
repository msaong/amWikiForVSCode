...
const exportGithub = (function () {
    return {
        ...
        _parseGithubUrl: function (url) {
            if (url === '') {
                alert('导出失败！    未检测到 “github-url” 配置 _(:з」∠)_    [请在 config.json 中配置您项目的 github-url]');
                return false;
            }
            if (url.indexOf('github.com') === -1) {
                alert('导出失败！    配置完整的 Github 项目 URL (；´д｀)ゞ    [例如 https://github.com/TevinLi/amWiki]');
                return false;
            }
            const urlArr = url.split('github.com')[1].split('/');
            if (urlArr.length < 3) {
                alert('导出失败！    请配置完整 Github 项目 URL o(*≧д≦)o!!    [例如 https://github.com/TevinLi/amWiki]');
                return false;
            }
